...
<div className={"competencesDescr " +colors}>Deuxième notion aprise en autodidacte mais il me manquait quelques notions importantes pour bien la maîtriser (<span className={"quote " +colors}>La puissance des différents display</span> par exemple). Ma formation m&apos;a permis de compléter ces notions et de savoir les maîtriser sans grande difficulté. </div>
<div className={"separatorBIG " +colors} id="Figma"></div>
<CompetenceSection name="Figma" pourcent="80" color="F86E60" />
<div className={"competencesDescr " +colors}>Complétement découvert pendant ma formation, cet outil est un plaisir pour travailler avec des personnes ayant des idées précises de leur <span className={"quote " +colors}>design</span>. Il permet aussi de partager des idées et de travailler efficacement en équipe sur des maquettes.</div>
<div className={"separatorBIG " +colors} id="REDUX"></div>
<CompetenceSection name="REDUX" pourcent="70" color="815ABD"/>
<div className={"competencesDescr " +colors}>Le premier state manager que j&apos;ai pu apréhender durant ma formation. Cet outil permet vraiment de simplifier le stockage de données et des préferences utilisateurs par exemple. (Un top pour éviter le <span className={"quote " +colors}>Props Drilling</span> quand on travaille avec REACT)
</div>
<div className={"separatorBIG " +colors} id="JS"></div>
<CompetenceSection name="JS" pourcent="60" color="EFD80B"/>
<div className={"competencesDescr " +colors}>Ce language est la base du code frontend, il permet de <span className={"quote " +colors}>s&apos;amuser</span> avec le DOM et de donner une possibilité d&apos;interaction pour l&apos;utilisateur. Language trés permissif mais qui permet d&apos;avoir une première logique de language. Sa compléxité réside dans ses évolutions et ses nombreuses syntaxes qui existe encore sur internet. Ma première approche a été dans la conception d&aposun <span className={"quote " +colors}>Click and Collect</span> pour mon ancienne entreprise, mais aussi utilisé pour rendre les interfaces des sites développés plus amicales.</div>
...
<div className={"competencesDescr " +colors}>Avoir enfin du CSS avec une <span className={"quote " +colors}>logique de language</span>, qui plus est qui se marie bien avec le fonctionnement par composant de REACT. Cet outil est trés puissant, je serait vraiment partant pour le maîtriser au mieux dans l&apos;avenir</div>
<div className={"separatorBIG " +colors} id="Jest"></div>
<CompetenceSection name="Jest" pourcent="40" color="C41F2D"/>
<div className={"competencesDescr " +colors}>Ne <span className={"quote " +colors}>jamais sous estimer</span> l&apos;importance de tester son site internet, cet outil m&apos;a permis de ne plus chercher bêtement certaines erreur et de pouvoir mieux cibler les défaillances de certaines parties de mon code.</div>
<div className={"separatorBIG " +colors} id="PHP"></div>
<CompetenceSection name="PHP" pourcent="5" color="7377AE"/>
<div className={"competencesDescr " +colors}>Language très peu travaillé, je l&apos;ai utilisé pour les système de réservation ou de Click and Collect, ceci permettait de récupérer les données du formulaire et d&apos;envoyer un mail au destinataire désiré de manière automatique, avec une redirection vers une page de confirmation. En découvrant le PHP, je me suis rendu compte que la logique de language est <span className={"quote " +colors}>presque</span> similaire à celle du JS, et grâce à mon apprentissage du JS je n&apos;ai pas été perdu en le travaillant.</div>
